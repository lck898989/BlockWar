...
cc._RF.push(module, '72fabuoUeRP7p15fRpEZu3I', 'UserInfo', __filename);
// Scripts/UserInfo.js
...
cc.Class({
    ...
    //单机加载用户信息
    StandAloneLoad: function StandAloneLoad() {},
    //加载一个用户信息
    LoadUser: function LoadUser(iconUrl, nickname, nodeName, nodePicture) {
        // var self = this;
    ...
    //加载两个用户头像
    LoadTwoUser: function LoadTwoUser(iconUrl, iconUr2, nodeName1, nickname, nodePicture1, nodePicture2) {
    ...
    //加载三个用户头像
    LoadThreeUser: function LoadThreeUser(iconUrl, iconUr2, iconUr3, nodePicture1, nodePicture2, nodePicture3, nickname, nodeName1) {
        var self = this;
    ...
    //加载用户头像
    LoadUserPicture: function LoadUserPicture(iconUr, nodePicture) {
        // if(iconUr!="")
        // {
    ...
    showWorldRank: function showWorldRank() {
        ...
        // }
        //向服务器请求世界排行数据
        var url = "https://m5.ykplay.com/getWorldRankByName?openid=" + this.openid;
                ...
                if (responseData.result === 'ok') {
                    if (responseData.data != null) {
                        var _loop = function _loop(i) {
                            //获得openId
                            var openId = responseData.data[i].openid;
                            //用户名
                            var name = responseData.data[i].name;
                            console.log("in UserInfo name is ", name);
                            var avatarUrl = responseData.data[i].avatarUrl;
                            console.log("in UserInfo avatarUrl is ", avatarUrl);
                            //积分
                            var integral = responseData.data[i].integral;
                            console.log("in UserInfo integral is ", integral);
                            //显示世界排行榜数据
                            var worldNode = self.setPrefabPosition(self.worldRankPrefab, 0, 385 - i * offset, vsNode);
                            console.log("世界排行榜节点是：", worldNode);
    ...
    cacelShowWorldRank: function cacelShowWorldRank() {
        //关闭世界排行榜页面
    ...
    loadUserPictureByWx: function loadUserPictureByWx(iconUrl, nodePicture) {
        imageLoader.imageLoadToolForWechatGame(iconUrl, function (spriteFrame) {
            console.log("spriteFrame is ", spriteFrame);
            nodePicture.getComponent(cc.Sprite).spriteFrame = spriteFrame;
        });
    },
    LoadRivalPicture: function LoadRivalPicture(iconUr, nodePicture) {
        if (CC_WECHATGAME) {
            this.loadRivalPictureByWx(iconUr, nodePicture);
        }
    },
    loadRivalPictureByWx: function loadRivalPictureByWx(iconUrl, nodePicture) {
    ...
    //加载用户头像
    LoadUserPictureAsync: function LoadUserPictureAsync(iconUr, nodePicture) {
        if (iconUr != "") {
            var self = this;
            //返回一个Promise
    ...
    showAdvice: function showAdvice() {
        ...
        var bannerWidth = 300;
        if (CC_WECHATGAME) {
            var _winSize = wx.getSystemInfoSync();
            console.log("winSize is ", _winSize);
    ...
    createAdvertisement: function createAdvertisement() {
        if (CC_WECHATGAME) {
            ...
            this.advice.onLoad(function () {
                console.log("banner 广告加载成功");
            });
            this.advice.onError(function (err) {
                console.log("err is ", err);
            });
            //重新修剪广告位
            self.advice.onResize(function (res) {
     ...
     */
    postMessage: function postMessage(action, type, score) {
