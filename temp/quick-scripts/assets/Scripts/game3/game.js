(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/Scripts/game3/game.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '24c8aLykAlOGYdQL6PWq3n8', 'game', __filename);
// Scripts/game3/game.js
...
cc.Class({
    ...
    onLoad: function onLoad() {
        ...
        this.normalSpeed = 0.7, this.quickSpeed = 0.07, this.up = 10;
        this.lr = 12;
    ...
    //生成形状每一个节点设置为都为0行随机列
    createShape: function createShape(parentNode) {
        //该节点数组消除次数容易计分
        // this.removeTime = 0;
        this.cishu = 0;
     ...
     */
    showNextShape: function showNextShape(nextBlock, parentNode, trimSizeJson) {
        ...
        if (this.showNoticeHelp) {
            ...
            if (!this.gameOver) {
                ...
                if (this.time > this.xltime) {
                    if (this.cishu < 3) {
                        if (this.cishu === 0) {
                            if (this.map[0][this.nodeArr[2].getComponent("Stone").col] != 1) this.nodeArr[2].active = true;else this.gameOver = true;
    ...
    */
    createMatrix: function createMatrix(rows, cols, initial) {
        var arr = [];
    ...
    **/
    ...
    //改变该节点数组是否存在可消除的节点
    checkNodeArr: function checkNodeArr(nodeArr, removeTime, isNetWork) {
        if (nodeArr.length != 0) {
            if (nodeArr.length > 0) {
                //临时数组存放待消节点
                var waitQueue = [];
     ...
     */
    directorFind: function directorFind(typeArr, row, col, direction, type, node) {
        //存放这个方向的临时数组
        var directionArr = [];
        //45度和-135度方向检测
        var leftRow = row;
     ...
     */
    isCommonType: function isCommonType(row, col, type) {
